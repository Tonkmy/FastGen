...
# Sweep selected Wan2.1-T2V DMD2 8-step checkpoints on the first five OpenVid eval prompts.
...
LOG_DIR=${LOG_DIR:-$EXP_ROOT/logs/eval_5prompts_step8}
...
if [ -n "${CKPTS:-}" ]; then
    ...
    CKPT_LIST=(0000500 0001000 0001500 0002000)
fi
...
run_case() {
    ...
    if [ "$FORCE" = "1" ]; then
        ...
        rm -f "$LOG_DIR/${ckpt}_student_8step_"*.log
    fi
    ...
    mkdir -p "$save_dir"
    local log_file="$LOG_DIR/${ckpt}_student_8step_$(date +%Y%m%d_%H%M%S).log"

    echo "[$(date -Is)] START $ckpt 8-step student inference"
    echo "config=$CONFIG"
    ...
    torchrun --standalone --nproc_per_node=1 \
        ...
        - trainer.ddp=True \
          ...
          trainer.seed="$SEED" \
          log_config.name="wan21_t2v_dmd2_OpenVid_global_8_step8_eval5_${ckpt}" \
        2>&1 | tee "$log_file"
...
echo "[$(date -Is)] 8-step eval-5 sweep complete."
