...
# Student-only Wan2.1 MeanFlow inference from the 1500-step FSDP checkpoint.
...
RUN_DIR=FASTGEN_OUTPUT/fastgen/wan_mf/wan21_t2v_mf_openvid_wan21_latents_global_8
CKPT_PATH=${CKPT_PATH:-$RUN_DIR/checkpoints/0001500}
...
VIDEO_SAVE_DIR=${VIDEO_SAVE_DIR:-$RUN_DIR/inference/0001500_student}
LOG_DIR=${LOG_DIR:-$RUN_DIR/logs}
mkdir -p "$VIDEO_SAVE_DIR" "$LOG_DIR"

LOG_FILE="$LOG_DIR/infer_0001500_mf_$(date +%Y%m%d_%H%M%S).log"
...
torchrun --standalone --nproc_per_node=1 \
    ...
    - trainer.ddp=True \
      ...
      model.enable_preprocessors=True \
      log_config.name=wan21_t2v_mf_openvid_wan21_latents_global_8_infer_1500 \
    2>&1 | tee "$LOG_FILE"
